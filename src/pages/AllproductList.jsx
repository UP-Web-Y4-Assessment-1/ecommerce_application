...
const AllProductList = () => {
  ...
  const subcategorylist = useQuery(
    "subcategory",
    async () =>
      await axiosClient.get("all/category/subcategory").then(({ data }) => data)
  );
  ...
  useEffect(() => {
    ...
    axiosClient.get("product/size/list").then(({ data }) => {
      var allSizlist = [];
      data?.map((sizelist) => {
    ...
    axiosClient.get("product/color/list").then(({ data }) => {
      var allColorlist = [];
      data?.map((colorlist) => {
                  ...
                  <Container>
                    <Row className="">
                      <Col lg="12" className="srcdiv">
                        ...
                        <ul>
                          {subcategorylist.isSuccess &&
                            subcategorylist.data?.map((cate) => {
                              return (
                                <>
                                  <li>
                                    <button
                                      ...
                                      id={`cate${cate.id}`}
                                      onClick={() => categoryProduct(cate.id)}
                                    >
                                      {cate.name}
                                    </button>
                                  </li>
                                  <UncontrolledCollapse
                                    ...
                                    className={cate.id == category && "show"}
                                  >
                                    {cate.subcategory?.map((sub) => {
                                      return (
                                        <li
                                          ...
                                          onClick={() =>
                                            subCategoryProduct(sub.id)
                                          }
                                        >
                                          -
                                          <span className="testbd">
                                            {sub.name}
                                          </span>{" "}
                                        </li>
                                      );
                              ...
                              <>
                                <FormGroup check>
                                  <Input
                                    ...
                                    checked={
                                      color == selectColor ? true : false
                                    }
                                  />
                                  <Label check>
                                    <div
                                      className="colorcodeset"
