...
const Home = () => {
  ...
  const bannerlink = useQuery(
    "bannerlink",
    async () =>
      await axiosClient.get("featured/link/list").then(({ data }) => data)
  );

  const offerBannerdata = useQuery(
    "myDatas",
    async () => await axiosClient.get("offer/banner").then(({ data }) => data)
  );
  const categoryListdata = useQuery(
    "category",
    async () =>
      await axiosClient.get("product/category").then(({ data }) => data)
  );
  const trendingListdata = useQuery(
    "trending",
    async () =>
      await axiosClient
        .get("home/trending/product/get")
        .then(({ data }) => data.data)
  );
  const bestsellListdata = useQuery(
    "bestsell",
    async () =>
      await axiosClient.get("home/best/sell/product").then(({ data }) => data)
  );
  const newarrivaldata = useQuery(
    "newarrival",
    async () =>
      await axiosClient.get("home/new/arrival/product").then(({ data }) => data)
  );
  const populardata = useQuery(
    "popular",
    async () =>
      await axiosClient.get("home/popular/product/get").then(({ data }) => data)
  );
  const popularcategory = useQuery(
    "popularCategory",
    async () =>
      await axiosClient.get("product/popular/category").then(({ data }) => data)
  );
  const popularbrand = useQuery(
    "popularBrand",
    async () =>
      await axiosClient.get("product/top/brand").then(({ data }) => data)
  );
      ...
      <section className="categories_feature">
        <Container className="paddingsst">
          <div className="cate_wise_feature">
            {bannerlink.isLoading == false &&
              bannerlink.data?.map((banner_link, index) => {
                return (
                  <div className="cate_feature">
                    <a
      ...
      <section className="trending__products">
        <Container>
          <Row className="productlistCard">
            ...
            <Col lg="12" md="12" sm="12">
              {trendingListdata.isLoading == false && (
        ...
        <Container>
          <Row className="productlistCard">
            ...
            <Col lg="12">
              ...
              {newarrivaldata.isLoading == false && (
                  ...
                  popularcategory.data?.map((data, index) => {
                    return (
                      <div className="col-lg-6 col-md-6">
                        <Link
                          to={`/products/list/${data.id}/0/popular/0/0`}
                          className="w-100"
                        >
                          <Top10Card
