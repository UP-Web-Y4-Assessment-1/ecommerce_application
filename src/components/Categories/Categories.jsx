...
const Categories = (props) => {
  ...
  useEffect(() => {
    axiosClient.get(`all/subcategory`).then(({ data }) => {
  ...
  return (
    <>
      <div className="categories__card category_card_position">
          ...
          categoryList?.map((value, index) => {
            return (
              <div
                ...
                key={index}
                onMouseEnter={() => subcategoryset(subCategoryList)}
        ...
        {showSubcategory && (
          <div
            className="categories__card subcategory_list"
            ...
            {subcategory &&
              subcategory?.map((value, index) => {
                return (
                  ...
                  // </div> </Link>
                  <Link
                    to={`/products/list/${value.category_id}/${value.id}/0/0`}
                    className="liststyle"
                    key={index}
                  >
                    {" "}
        ...
        <div className="all_cate_product text-center">
          <Link to="/all/category"> View All </Link>
        </div>
      </div>
    </>
  );
