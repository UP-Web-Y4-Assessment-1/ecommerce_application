...
const OrderShippingAddress = (props) => {
  ...
  useEffect(() => {
    ...
    axiosClient.get("user/shipping/billing/address/get").then(({ data }) => {
      if (data.data) {
        ...
        if (data.data.shipping_district)
        ...
        setUserInfo({
          ...
          division: `${
            data.data.shipping_division == null
              ? 0
              : data.data.shipping_division
          }`,
          district: `${
            data.data.shipping_district == null
              ? 0
              : data.data.shipping_district
          }`,
        });

        dispatch(
          shippingCost({
            divisionId: userInfo.division,
            districtId: setUserInfo.district,
          })
        );
      }
    });
  ...
  let onChangeDistrict = (event) => {
    try{
      ...
      console.log("My District: " + event.target.value);
      inputdata(event);
      dispatch(
        shippingCost({
          divisionId: userInfo.division,
  ...
  return (
    <>
      ...
      <div>
      <button className="buy__btn auth__btn w-100" onClick={placeOrder}>
  Shipping
</button>
      </div>
    </>
