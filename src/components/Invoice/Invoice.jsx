...
const Invoice = (props) => {
  ...
  useEffect(() => {
    if (props.order_id > 0) {
      axiosClient
        .get(`user/order/details/by/orderId?id=${props.order_id}`)
        .then(({ data }) => {
      ...
      <Table className="tablesst mt-4">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td className="flotlert-set">0</td>
          </tr>
          <tr>
            <th colSpan={3}></th>
            <td className="textright_st">
...
const InvoiceDetails = (props) => {
  return (
    <tr>
      <th scope="row">{props.si}</th>
      <td style={{ width: "40%" }}>{props.data.product_name}</td>
      <td>{props.data.sale_quantity}</td>
      <td>
